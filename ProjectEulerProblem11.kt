fun main(args: Array<String>) {
    ...
    for(row in 0..19){
        for(column in 0..16){
            multiply = Grid[row][column] * Grid[row][column+1] * Grid[row][column+2] * Grid[row][column+3]
    ...
    for(column in 0..19){
        for(row in 0..16){
            multiply = Grid[row][column] * Grid[row+1][column] * Grid[row+2][column] * Grid[row+3][column]
            if(multiply>max) max = multiply
        }
    }
    ...
    for(row in 0..16){
        for(column in 0..16){
            multiply = Grid[row][column] * Grid[row+1][column+1] * Grid[row+2][column+2] * Grid[row+3][column+3]
            if(multiply>max) max = multiply
        }
    }
